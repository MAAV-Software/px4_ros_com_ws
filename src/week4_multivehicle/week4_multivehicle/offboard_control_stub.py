#!/usr/bin/env python3
"""Starter scaffold — week 4 onboarding exercise (task 3).

This is your week 3 offboard control node, adapted to target one specific
vehicle instance in a 2-instance SITL setup. The arm/takeoff/waypoint/land
control logic from week 3 is already filled in below — that's last week's
skill. This week's new piece is topic namespacing, left as a TODO in topic().

  TODO(task 3): implement topic() so it returns the correctly namespaced
  topic name for self.vehicle_instance.

Background: PX4 SITL namespaces every /fmu/... topic with `px4_<instance>`
for any instance other than 0 (see ROMFS/px4fmu_common/init.d-posix/rcS in
PX4-Autopilot — instance 0 gets no prefix, instance N gets "px4_N"). Compare
against week4_multivehicle/offboard_control_solution.py once you've got it working.

Usage:
  ros2 run week4_multivehicle offboard_stub --ros-args -p vehicle_instance:=1
...
class OffboardControl(Node):
    ...
    def topic(self, suffix):
        ...
        '/fmu/out/vehicle_status'. Instance N (N != 0) should be prefixed
        with px4_N, e.g. '/px4_1/fmu/out/vehicle_status'.
        """
    ...
    def on_timer(self):
        # Same control logic as week 3 — this part shouldn't need to change.
        if self.landed:
            return
