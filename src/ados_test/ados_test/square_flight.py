#!/usr/bin/env python3
"""ados_test: fly a 50x50 yard square, 5 yards above the ground.

Arms, takes off to 5 yards AGL, flies the square waypoint path defined in
resource/waypoints.yaml, then lands back at the start corner. Same
arm/offboard/waypoint pattern as week3_offboard_control's reference
solution, but the path always comes from the yaml file instead of being
hardcoded.
"""
import os
...
def load_waypoints(package_name='ados_test', filename='waypoints.yaml'):
...
class SquareFlight(Node):
    def __init__(self):
        super().__init__('ados_test_square_flight')

        self.waypoints = load_waypoints()
        self.get_logger().info(f'loaded {len(self.waypoints)} waypoints from waypoints.yaml')
    ...
    def on_timer(self):
        ...
        # mode - this is why the logic lives in a timer callback.
